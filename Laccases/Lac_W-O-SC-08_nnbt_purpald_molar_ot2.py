"""
W-O-SC-08  NNBT / Purpald laccase HTS - MOLAR dilution, inputs from the app          Kristine Toft Johansen s215098
=======================================================================================
...
def add_parameters(parameters):
    ...
    # --- the batch: no need to open the file ---------------------------------------
    parameters.add_int(
        display_name='Number of enzymes', variable_name='n_enz', default=3,
        minimum=0, maximum=MAX_PARAM_ENZYMES,
        description='How many enzyme slots below are in use. 0 = curve-only pilot.')
    parameters.add_float(
        display_name='Target conc (uM)', variable_name='target_um', default=0.0,
        minimum=0.0, maximum=1000.0,
        description='Molar target every enzyme is diluted to. 0 = auto.')
    for i in range(1, MAX_PARAM_ENZYMES + 1):
        parameters.add_float(
            display_name=f'{i}. Bradford (mg/mL)', variable_name=f'mg_ml_{i}',
            default=1.00, minimum=0.0, maximum=100.0,
            description=f'{ENZYME_NAMES[i - 1]}, tube {tube_well_name(i - 1)}.')
        parameters.add_float(
            display_name=f'{i}. Molar mass (kDa)', variable_name=f'mw_{i}',
            default=65.0, minimum=1.0, maximum=500.0,
            description=f'{ENZYME_NAMES[i - 1]} molar mass.')
    parameters.add_float(
        display_name='Heat-inact (mg/mL)', variable_name='heat_mg_ml', default=2.40,
        minimum=0.0, maximum=100.0, description='0 = leave the control off the plate.')
    parameters.add_float(
        display_name='Heat-inact (kDa)', variable_name='heat_mw', default=65.0,
        minimum=1.0, maximum=500.0, description='Heat-inactivated control molar mass.')
...
metadata = {
    'apiLevel': '2.20',
    'protocolName': 'W-O-SC-08 - NNBT Purpald laccase HTS (auto-dilution + standard curve)',
    'description': (
...
# =======================================================================================
# Tubes go on the rack in THIS ORDER: A1, B1, C1, D1, A2, ...
# Bradford readings and molar masses are RUNTIME PARAMETERS - typed into the Opentrons
# app at run setup. What is below is only what the local preview uses; ENZYME_NAMES are
# the plate-map labels, because the app cannot take free text.
MAX_PARAM_ENZYMES = 8                  # how many enzyme slots the app offers
ENZYME_NAMES = ['Lac-01', 'Lac-02', 'Lac-03', 'Lac-04',
                'Lac-05', 'Lac-06', 'Lac-07', 'Lac-08']

# name, Bradford reading mg/mL, molar mass kDa. Preview defaults only.
ENZYME_BATCH_DEFAULT = [
    {'name': 'Lac-01', 'mg_ml': 2.40, 'mw_kda': 65.0},
    {'name': 'Lac-02', 'mg_ml': 1.85, 'mw_kda': 65.0},
    {'name': 'Lac-03', 'mg_ml': 3.10, 'mw_kda': 70.0},
]

# mg/mL is a MASS concentration: at equal mg/mL a heavier enzyme is present as fewer
# molecules. Dividing by molar mass converts to molarity, so every enzyme enters the
# assay at the same NUMBER of molecules - which is what an activity comparison needs.
#   uM = mg/mL / kDa x 1000      e.g. 2.40 mg/mL of a 65 kDa enzyme = 36.9 uM
ENZ_TARGET_UM = None                   # target in uM; None = auto (in the app, 0 = auto)
ENZ_TARGET_AUTO_FRACTION = 0.90        # auto = this x the weakest enzyme, in uM

HEAT_INACT_NAME = 'Heat-inactivated'   # negative control: killed enzyme, tube C6
HEAT_INACT_MG_ML = 2.40                # preview defaults; the app asks for these too
HEAT_INACT_MW_KDA = 65.0
INCLUDE_HEAT_INACT = True
...
def as_um_batch(batch):
    """[{'name','mg_ml','mw_kda'}] -> [{'name','conc'}] with conc in uM."""
    return [{'name': e['name'], 'conc': to_um(e['mg_ml'], e['mw_kda']),
             'mg_ml': e['mg_ml'], 'mw_kda': e['mw_kda']} for e in batch]


def resolve_target_conc(enzymes, heat_um=None, target_um=None):
    """The common MOLAR concentration (uM) every enzyme is diluted down to."""
    if target_um:                                    # 0 or None means auto
        return float(target_um)
    # In pilot mode there may be no enzymes; the heat-inactivated control is then the
    # only protein on deck, so it sets the target. With neither, the value is unused.
    measured = [e['conc'] for e in enzymes]
    if heat_um is not None:
        measured.append(heat_um)
    if not measured:
        return 0.0
...
def build_layout(batch=None, target_um=None, include_heat=None,
                 heat_mg_ml=None, heat_mw_kda=None):
    ...
    `batch` is [{'name','mg_ml','mw_kda'}]; everything downstream works in uM."""
    batch = ENZYME_BATCH_DEFAULT if batch is None else batch
    include_heat = INCLUDE_HEAT_INACT if include_heat is None else include_heat
    heat_mg_ml = HEAT_INACT_MG_ML if heat_mg_ml is None else heat_mg_ml
    heat_mw_kda = HEAT_INACT_MW_KDA if heat_mw_kda is None else heat_mw_kda
    ENZYME_BATCH = as_um_batch(batch)                # uM from here on
    heat_um = to_um(heat_mg_ml, heat_mw_kda) if include_heat else None
    INCLUDE_HEAT_INACT_L = include_heat
    HEAT_INACT_CONC = heat_um
    ...
    target = resolve_target_conc(ENZYME_BATCH, heat_um,
                                 ENZ_TARGET_UM if target_um is None else target_um)
    ...
    gradient = build_gradient_plan(n_enz + (1 if INCLUDE_HEAT_INACT_L else 0))
    for g in gradient:
...
def render_summary(layout):
    ...
    out.append(f'  BATCH: {len(layout["dilutions"])} enzymes, {len(layout["gradient"])} '
...
def run(protocol):
    ...
    # -----------------------------------------------------------------------------
    # Batch straight from the app: mg/mL + kDa in, uM out. LAYOUT is LOCAL to run(),
    # shadowing the module-level default that the local preview uses.
    prm = protocol.params
    batch = [{'name': ENZYME_NAMES[i], 'mg_ml': getattr(prm, f'mg_ml_{i + 1}'),
              'mw_kda': getattr(prm, f'mw_{i + 1}')} for i in range(prm.n_enz)]
    LAYOUT = build_layout(batch, target_um=prm.target_um,
                          include_heat=prm.heat_mg_ml > 0,
                          heat_mg_ml=prm.heat_mg_ml or 1.0,
                          heat_mw_kda=prm.heat_mw)
    for e in as_um_batch(batch):
        protocol.comment(f'    {e["name"]}: {e["mg_ml"]:g} mg/mL / {e["mw_kda"]:g} kDa '
                         f'= {e["conc"]:.2f} uM')
    protocol.comment(f'    diluting everything to {LAYOUT["target_conc"]:.2f} uM')
